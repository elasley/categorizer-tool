...
const BulkAssignmentTool = ({
  ...
  const [bulkAction, setBulkAction] = useState("reassign");
  ...
  const filterOptions = useMemo(() => {
    const brands = [
      ...new Set(products.map((p) => p.brand || p.Brand).filter(Boolean)),
    ].sort();
    const categories = [
      ...new Set(products.map((p) => p.suggestedCategory).filter(Boolean)),
    ].sort();
    const statuses = [
  ...
  const filteredProducts = useMemo(() => {
    let filtered = products.filter((product) => {
      ...
      if (filters.confidence !== "all") {
        switch (filters.confidence) {
          case "high":
            ...
            break;
          case "medium":
            if (confidence < 40 || confidence >= confidenceThreshold)
              return false;
            break;
          case "low":
            if (confidence >= 40 || confidence === 0) return false;
            break;
          case "none":
      ...
      if (
        ...
        product.suggestedCategory !== filters.category
      )
        return false;
    ...
    if (smartSelectionMode) {
      filtered = filtered.filter((product) => {
        const confidence = product.confidence || 0;
        if (
          confidence < selectionRules.minConfidence ||
          confidence > selectionRules.maxConfidence
        ) {
          return false;
        }
        ...
        if (selectionRules.keywords) {
          const productText = [product.name, product.description]
            .join(" ")
            .toLowerCase();
          const keywords = selectionRules.keywords
            .toLowerCase()
            .split(",")
            .map((k) => k.trim());
        ...
        if (selectionRules.categories.length > 0) {
          if (!selectionRules.categories.includes(product.suggestedCategory))
            return false;
        }

        if (
          selectionRules.excludeManual &&
  ...
  const stats = useMemo(() => {
    ...
    const highConfidence = filteredProducts.filter(
      (p) => (p.confidence || 0) >= confidenceThreshold
    ).length;
    const needsReview = filteredProducts.filter(
      (p) => (p.confidence || 0) < 50 && (p.confidence || 0) > 0
    ).length;
    const uncategorized = filteredProducts.filter(
      (p) => !p.suggestedCategory
    ).length;
    const avgConfidence =
      total > 0
        ? filteredProducts.reduce((sum, p) => sum + (p.confidence || 0), 0) /
          total
        : 0;
  ...
  const applySmartSelection = () => {
    ...
    filteredProducts.forEach((product) => {
      ...
      if (
        ...
        if (
          selectionRules.brands.length === 0 ||
          selectionRules.brands.includes(product.brand || product.Brand)
        ) {
          if (
            !selectionRules.keywords ||
            [product.name, product.description]
              .join(" ")
              .toLowerCase()
              .includes(selectionRules.keywords.toLowerCase())
          ) {
            if (
              selectionRules.categories.length === 0 ||
              selectionRules.categories.includes(product.suggestedCategory)
            ) {
              if (
                !selectionRules.excludeManual ||
                product.status !== "manual-assigned"
  ...
  const handleBulkDelete = () => {
    if (
      window.confirm(
        `Are you sure you want to remove ${selectedProducts.size} products from categorization? This will reset their categories to uncategorized.`
      )
    ) {
      const selectedIds = Array.from(selectedProducts);
      const updatedProducts = products.map((product) => {
      ...
      setSelectedProducts(new Set());
      alert(
        `Successfully removed categorization from ${selectedIds.length} products`
      );
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-7xl h-5/6 flex flex-col">
        ...
        <div className="flex h-[340px] ">
          ...
          <div className="w-80 border-r border-gray-200 flex flex-col flex-shrink-0 overflow-y-auto">
            {/* Search and Filters */}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  >
                    ...
                    <option value="medium">
                      Medium (40-{confidenceThreshold - 1}%)
                    </option>
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`p-1 rounded ${
                    smartSelectionMode ? "text-purple-600" : "text-gray-400"
                  }`}
                >
                ...
                {bulkAction === "reassign" && (
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {targetCategory &&
                        Object.keys(categories[targetCategory] || {}).map(
                          (subcat) => (
                    ...
                    >
                      ...
                      {targetSubcategory &&
                        (
                          categories[targetCategory]?.[targetSubcategory] || []
                        ).map((part) => (
  ...
  return (
    <div
      className={`p-4 hover:bg-gray-50 cursor-pointer transition-colors ${
        ...
        showPreview && isSelected && bulkAction === "reassign"
          ? "bg-green-50"
          : ""
      }`}
          ...
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span
              className={`text-sm font-bold ${
                (product.confidence || 0) >= confidenceThreshold
                  ? "text-green-600"
              ...
              {bulkAction === "reassign" && targetPartType && (
                <div className="p-2 bg-green-100 border border-green-200 rounded text-xs">
                  <div className="flex items-center space-x-2 text-green-800">
                    ...
                    <span>
                      Will be assigned to: {targetCategory} →{" "}
                      {targetSubcategory} → {targetPartType}
                    </span>
                  </div>
                </div>
              )}
